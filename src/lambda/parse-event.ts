...
function detectService(event: any) {
    ...
    if (
        process.env.KUBIKS_TRACE_STEP_FUNCTION === "true" ||
        event.Payload?._kubiks?.traceparent || event._kubiks?.traceparent ||
        (Array.isArray(event) && (event[0]?.Payload?.kubiks?.traceparent || event[0]?._kubiks?.traceparent))
    ) {
